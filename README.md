...
תכונה חדשה: במודל הוספה/עריכה של פער חובה לרשום גם את שם ומספר הטלפון של מי
שפתח אותו (`opened_by_name`, `opened_by_phone`). הפרטים מוצגים במסך "פרטי
הפער" (🕐), והטלפון מוצג כקישור שאפשר ללחוץ עליו כדי להתקשר ישירות מהנייד.
...
## עדכון סכימה — מבנה ומספר חדר

נוסף מיקום חדש לרשימה — **כיתות**. כשבוחרים מיקום מסוג **מגורי בנות**,
**מגורי בנים** או **כיתות**, נפתחים במודל הוספה/עריכה עוד שני שדות: **מבנה**
ו‑**מספר חדר** (שני השדות רשותיים — `building`, `room_number`), כדי לדעת
בדיוק איפה בתוך המבנה קרה הפער. הפרטים מוצגים גם בשורת הטבלה הראשית (מתחת
למיקום) וגם במסך "פרטי הפער", וגם ניתן לחפש לפיהם בתיבת החיפוש.
...
## שדות חובה במודל הוספה/עריכה

כל השדות במודל הוספה/עריכה של פער הם שדות חובה, **חוץ מ**הערות, מבנה
ומספר חדר. אם לוחצים על "שמירה"/"הוספה" ושדה חובה ריק, השדה מקבל מסגרת
אדומה כדי לסמן שצריך למלא אותו, והשמירה לא מתבצעת עד שכל שדות החובה מלאים.

---

## עמודת "עודכן לאחרונה"

בטבלה (ובתצוגת הכרטיסיות) נוספה עמודה שמציגה מתי בפעם האחרונה נערך שינוי
בפער — כולל שעה, לא רק תאריך. אפשר גם למיין לפיה (כפתור המיון ⇅ בכותרת),
והיא כלולה גם בייצוא לאקסל. השדה מתעדכן אוטומטית על ידי מסד הנתונים בכל
שמירה, אין צורך לגעת בו ידנית.

---

## עדכון סכימה — עדכונים חופשיים לכל פער

תכונה חדשה: במסך "פרטי הפער" (🕐) יש עכשיו אזור **עדכונים**, נפרד
מהיסטוריית השינויים הטכנית. כל איש צוות יכול לכתוב שם עדכון חופשי וקצר —
למשל "הוזמן טכנאי, מגיע מחר" — והוא נשמר עם תאריך/שעה ושם מי שכתב אותו.
אפשר גם למחוק עדכון בטעות שנכתב לא נכון.

העדכון האחרון של כל פער מוצג גם בתקציר קצר ישירות בשורת הטבלה הראשית
(מתחת לשם הפער, עם סמל 🔧), כך שאפשר לראות שיש עדכון חדש בלי לפתוח את
הפער. הכל מסונכרן בזמן אמת — כל מי שהדף פתוח אצלו יראה עדכון חדש מיד,
בלי צורך בהתראות SMS/וואטסאפ/מייל.

זה דורש טבלה חדשה במסד הנתונים — `gap_updates`. אם ה‑DB שלכם הוקם לפני
התוספת הזו: חוזרים ל‑Supabase **SQL Editor** ומריצים שוב את כל `schema.sql`
(בטוח לחזרה — `create table if not exists`). בלי הריצה הזו, הוספה/עריכה
של פער עדיין תעבוד, אבל אזור העדכונים במסך "פרטי הפער" לא יעבוד.
